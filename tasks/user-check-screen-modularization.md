...
- [ ] 신규 폴더 생성 및 위젯 파일 스캐폴딩.
- [ ] `UserCardItem` 및 `UserActionButtons` 추출 (복잡한 조건부 렌더링 캡슐화).
- [ ] `UserDeleteDialog` 별도 정적 유틸 또는 위젯으로 분리.
- [ ] **[Git Commit]**: 물리 위젯 분리 완료.

### Phase 2: 상태 관리 고도화 (Selector 패턴 적용)
- [ ] `UserCheckScreen` 메인 위젯에서 `Selector`를 사용하여 리스트 데이터 변화만 감시.
- [ ] 검색(onChanged) 시 무거운 위젯 레이어 리빌드 차단.
- [ ] **[Git Commit]**: 부분 리빌드 성능 최적화 완료.

### Phase 3: 프리미엄 디자인 고도화 (Visual Excellence)
- [ ] `NeoTheme` 스타일을 적극 활용한 카드 배경 및 광택 효과 강화.
- [ ] 버튼 클릭 시 `AnimatedScale` 또는 피드백 효과 추가.
- [ ] **[Git Commit]**: 디자인 시스템 정밀 적용 완료.

### Phase 4: 완결성 검증 및 빌드
- [ ] `flutter analyze`를 통한 린트 에러 제로 확인.
- [ ] 관리자 앱 APK 디버그 빌드 (`flutter build apk --debug`).
- [ ] **마지막 Git Commit**: "Optimize UserCheckScreen with modular widgets and Selector".

## 3. 정합성 최종 체크리스트
- [ ] `user_check_screen.dart` 메인 소스가 150줄 이내로 경량화되었는가?
- [ ] 사용자 삭제/승인/거절 시 스낵바 및 결과 피드백이 정상인가?
- [ ] `Selector` 도입으로 검색 속도가 눈에 띄게 개선되었는가?
