...
### Phase 1: 위젯 추출 및 구조화
- [ ] `ActiveTagEditor` 클래스 생성 및 `SmartTagImageSection`에서 로직 이관.
- [ ] 부모 위젯에서 자식 위젯으로 필요한 콜백 및 데이터 전달 구조 정립.
- [ ] **[Git Commit]**: 물리 분리 완료.

### Phase 2: 성능 고도화 (State Management Optimization)
- [ ] `SmartTagImageSection` 내부에 `Selector` 위젯 적용.
- [ ] `TreeRegistrationViewModel`에서 필요한 최소 데이터(activeTag, image)만 감시하도록 수정.
- [ ] **[Git Commit]**: Selector 패턴 적용 완료.

### Phase 3: 모바일 렌더링 부하 분산
- [ ] 이미지 업로드 영역 및 미리보기 영역에 `RepaintBoundary` 적용.
- [ ] `TagHintInput`의 `TextEditingController` 격리 상태 재검증.
- [ ] **[Git Commit]**: 렌더링 최적화 완료.

### Phase 4: 최종 검증 및 빌드
- [ ] `flutter analyze` 실행 및 린트 에러 제로 확인.
- [ ] `flutter build apk --debug` 수행하여 런타임 안정성 체크.
- [ ] **마지막 Git Commit**: "Optimize SmartTagImageSection with Selector and RepaintBoundary".

## 3. 정합성 최종 체크리스트
- [ ] `SmartTagImageSection.dart`가 60줄 이하로 경량화되었는가?
*   [ ] 다른 필드 입력 시 이미지 위젯의 불필요한 리빌드가 차단되었는가?
- [ ] `RepaintBoundary`가 적절한 위치에 배치되었는가?
