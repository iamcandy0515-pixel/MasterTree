      ...
      children: [
        Row(
          ...
          children: [
            const Row(
              children: [
                Icon(Icons.settings_input_component, color: primaryColor, size: 20),
                SizedBox(width: 8),
                Text(
                  '추출 조건 설정',
                  style: TextStyle(
        ...
        Container(
          ...
          child: Column(
            children: [
              Row(
                children: [
                  Expanded(child: _buildInfoField('과목', vm.selectedSubject ?? '미선택')),
                  const SizedBox(width: 12),
                  Expanded(child: _buildInfoField('연도', vm.selectedYear?.toString() ?? '미선택')),
                  const SizedBox(width: 12),
                  Expanded(child: _buildInfoField('회차', vm.selectedRound?.toString() ?? '미선택')),
                ],
              ),
