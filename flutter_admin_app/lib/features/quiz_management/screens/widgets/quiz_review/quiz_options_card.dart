...
class _QuizOptionsCardState extends State<QuizOptionsCard> {
  ...
  void initState() {
    ...
    _correctController.addListener(
        () => widget.onCorrectOptionChanged(_correctController.text));
    _incorrectControllers = widget.incorrectOptions
        .map((opt) => TextEditingController(text: opt))
        .toList();

    for (int i = 0; i < _incorrectControllers.length; i++) {
      _incorrectControllers[i].addListener(() =>
          widget.onIncorrectOptionChanged(i, _incorrectControllers[i].text));
    }
  }
  ...
  void didUpdateWidget(QuizOptionsCard oldWidget) {
    ...
    if (widget.incorrectOptions.length != _incorrectControllers.length) {
      ...
      _incorrectControllers.addAll(widget.incorrectOptions
          .map((opt) => TextEditingController(text: opt)));
      for (int i = 0; i < _incorrectControllers.length; i++) {
        _incorrectControllers[i].addListener(() =>
            widget.onIncorrectOptionChanged(i, _incorrectControllers[i].text));
    ...
    return Column(
      ...
      children: [
          ...
          children: [
            ...
            if (widget.isGenerating)
              ...
              _buildAIAssistantButton(
                  'AI 오답 생성', Icons.auto_awesome, widget.onAiGenerate, aiColor),
          ],
        ),
        ...
        ..._incorrectControllers.asMap().entries.map((e) => Padding(
              ...
              child:
                  _buildOptionField('오답 ${e.key + 1}', e.value, isCorrect: false),
            )),
      ],
    );
  }
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        labelStyle: TextStyle(
            color: isCorrect ? primaryColor : Colors.white54, fontSize: 12),
        filled: true,
        fillColor: surfaceDark,
        enabledBorder: OutlineInputBorder(
            borderRadius: BorderRadius.circular(8),
            borderSide: BorderSide(
                color: isCorrect ? primaryColor.withOpacity(0.3) : Colors.white10)),
        focusedBorder: OutlineInputBorder(
            borderRadius: BorderRadius.circular(8),
            borderSide:
                BorderSide(color: isCorrect ? primaryColor : Colors.white38)),
      ),
    );
  }

  Widget _buildAIAssistantButton(
      String label, IconData icon, VoidCallback onPressed, Color aiColor) {
    return InkWell(
      onTap: onPressed,
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Text(label,
                style: TextStyle(
                    color: aiColor, fontSize: 12, fontWeight: FontWeight.bold)),
          ],
        ),
      ),
    );
