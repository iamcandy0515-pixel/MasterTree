...
class QuizRelatedCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final int displayPage = (currentPage >= totalPages && totalPages > 0)
        ? totalPages - 1
        : currentPage;
    final int startIndex = displayPage * itemsPerPage;
    final int endIndex = (startIndex + itemsPerPage < relatedQuizzes.length)
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text('유사 문제 목록 (${relatedQuizzes.length}건)',
                ...
                children: [
                  if (isRecommending)
                    ...
                    _buildAIAssistantButton(
                        'AI 추천', Icons.auto_awesome, onAiRecommend, aiColor),
  ...
  Widget _buildRelatedItem(Map<String, dynamic> quiz, Color primaryColor) {
    ...
    if (blocks != null && blocks.isNotEmpty) {
      ...
      if (textBlock != null) {
        content =
            textBlock['content']?.toString().replaceAll('\n', ' ').trim() ?? '';
      }
    }
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Text('$year년 $round회 $qNo번 $category)',
                style: TextStyle(
                    color: primaryColor,
                    fontSize: 10,
                    fontWeight: FontWeight.bold),
