...
1. [ ] **[0-1] 로컬 Git 백업**: 현재 상태 git stash 또는 commit. (완료)
2. [ ] **[Backend] Cloudinary 초기 설정**: .env 변수 로딩 및 SDK 초기화 로직 점검.
3. [ ] **[Backend] UploadService 리팩토링**: Supabase Storage 연동부를 Cloudinary로 1:1 교체.
4. [ ] **[Backend] 린트 체크**: 
pm run lint 수행하여 타입 에러 제거.
5. [ ] **[Validation] 통합 테스트**: 관리자 앱 -> 서버 업로드 -> Cloudinary 저장 -> DB 기록 확인.
...
---
**상태**: **승인 대기 중 (Implementation Deferred)**
