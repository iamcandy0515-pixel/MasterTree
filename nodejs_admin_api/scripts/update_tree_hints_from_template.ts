...
async function updateTreeHintsFromTemplate() {
    console.log("🔄 Updating tree hints from template...\n");

    // Load template data
    const dataPath = path.resolve(__dirname, "data/tree_hints_template.json");

    if (!fs.existsSync(dataPath)) {
        console.error(`❌ Template file not found: ${dataPath}`);
        console.error("Please create tree_hints_template.json first");
        process.exit(1);
    }

    const rawData = fs.readFileSync(dataPath, "utf-8");
    const hintsData: TreeHintUpdate[] = JSON.parse(rawData);

    console.log(`📦 Loaded ${hintsData.length} trees from template\n`);
